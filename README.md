...
This fork adds a first-class multi-language switcher and a Go embedding
package, so the same API can be served in several languages (for example the
documents generated by `swag init --llm`).

### JavaScript configuration

Use the `languages` option instead of (or in addition to) `urls`. The Topbar
renders a **Select a language** dropdown and the choice is persisted in the
`languages.primaryName` query parameter.

```js
SwaggerUIBundle({
  languages: [
    { name: "English", url: "/swagger/doc.json" },
    { name: "中文",   url: "/swagger/zh-CN/doc.json" },
    { name: "日本語", url: "/swagger/ja/doc.json" },
  ],
...
```console
go get github.com/holihur/swagger-ui
```

Mount it with `net/http` (or `gin.WrapH`, `echo.WrapHandler`, ...):
...
When `SpecProvider` and `Language.InstanceName` are set, the handler serves
the OpenAPI documents itself at `/swagger/doc/<instanceName>.json`, so no
additional routes are required.

`swaggerui.FS` exposes the embedded distribution if you prefer to build your
own handler.
