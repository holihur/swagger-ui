...
<a name="languages"></a>`languages` | `LANGUAGES` | `Array`. An array of language objects (`[{url: "<url1>", name: "<name1>"},{url: "<url2>", name: "<name2>"}]`) used by the Topbar plugin to render a language switcher. It works like `urls` but selects the API definition to display by language, which is useful for serving the same API in multiple languages. When used and the Topbar plugin is enabled, the `url` parameter will not be parsed. Names and URLs must be unique among all items in this array, since they're used as identifiers.
<a name="languages.primaryName"></a>`languages.primaryName` | `LANGUAGES_PRIMARY_NAME` | `String`. When using `languages`, you can use this subparameter. If the value matches the name of a language provided in `languages`, that language will be displayed when Swagger UI loads, instead of defaulting to the first entry in `languages`.
